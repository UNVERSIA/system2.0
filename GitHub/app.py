...
def detect_and_convert_data(file_path_or_buffer):
    ...
    if is_daily_data:
        ...
        if '处理水量_raw' in df.columns:
            ...
            conversion_info.append(f"  • 处理水量: 日数据 × 30 = 月数据 (示例: {df['处理水量_raw'].iloc[0]:,.0f} × 30 = {df['处理水量(m³)'].iloc[0]:,.0f})")
        
        # 电耗：日 -> 月
        if '电耗' in df.columns:
            df['电耗(kWh)'] = df['电耗'] * 30
            conversion_info.append(f"  • 电耗: 日数据 × 30 = 月数据 (示例: {df['电耗'].iloc[0]:,.0f} × 30 = {df['电耗(kWh)'].iloc[0]:,.0f})")
        
        # PAC：日 -> 月
        if 'PAC_raw' in df.columns:
            df['PAC投加量(kg)'] = df['PAC_raw'] * 30
            conversion_info.append(f"  • PAC: 日数据 × 30 = 月数据 (示例: {df['PAC_raw'].iloc[0]:,.1f} × 30 = {df['PAC投加量(kg)'].iloc[0]:,.1f})")
...
with tab5:
    ...
    with predict_col1:
        ...
        st.info(f"预测范围: {forecast_start.strftime('%Y年%m月')} 至 {forecast_end.strftime('%Y年%m月')}（{prediction_months}个月）")
        ...
        with st.spinner(f"正在进行未来{prediction_months}个月预测..."):
            try:
                ...
                try:
                    ...
                    # 计算历史标准差用于生成合理的波动
                    historical_std = df_calc['total_CO2eq'].std() if 'total_CO2eq' in df_calc.columns else historical_avg * 0.1
                    ...
                    for i in range(prediction_months):
                        ...
                        random_noise = np.random.normal(0, noise_magnitude / historical_avg if historical_avg > 0 else 0.05)
                        ...
                        # 组合所有成分
                        pred = historical_avg * seasonal_factor * trend_factor * (1 + random_noise + month_offset) + compound_variation * historical_avg
                    ...
                    try:
                        ...
                        if 'predicted_CO2eq' in prediction_data.columns:
                            ...
                            if recent_historical_avg > 0 and predicted_monthly_avg > 0:
                                ...
                                # 获取预测期间说明
                                forecast_start_str = prediction_data['日期'].min().strftime('%Y年%m月') if '日期' in prediction_data.columns else '未来'
                                forecast_end_str = prediction_data['日期'].max().strftime('%Y年%m月') if '日期' in prediction_data.columns else '期间'
